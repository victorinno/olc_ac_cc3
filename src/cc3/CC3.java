...
//			a.getIventorio().getArmas()[0].ativar(a);
			...
			/* + a.getIventorio().getArmas()[0].getAtaque() */) + "/" + a.getNome() + " ataca"/*om "*/
					/*+ a.getIventorio().getArmas()[0].getNome()*/);
		} catch (FileNotFoundException ex) {
			Logger.getLogger(CC3.class.getName()).log(Level.SEVERE, null, ex);
...
//			int apup = a.getIventorio().getAcessorios()[0].pup();
			a.setPowerUp(p.getNivel() == 1 ? a.getPowerUp() - 0 : a.getPowerUp() - p.getNivel() /* + apup */);
			System.out.println("Powerup - " + p.getNivel() + " = " + a.getPowerUp() /* + apup */);
...
//			a.getIventorio().getAcessorios()[0].ativar(a);
			a.setDefenseUp(p.getNivel() == 1 ? a.getDefenseUp() : a.getDefenseUp() - p.getNivel());
...
//			int adup = a.getIventorio().getAcessorios()[0].dup();
			...
			if (rolagem < chance) {
				...
				Integer dano = (int) (new Integer(atks[1]) / resistencia + ((Math.random() * 4) + 1)) + cri;
				dano = dano < 1 ? 1 : dano;
	...
	public void evolucaoPoder(Integer poder, Lutador a) {
		...
		if (cf <= p.getChanceForca()) {
			...
			System.out.println("Poder: " + p.getNome() + " aumentou em forca!");
		}

		if (cp <= p.getChancePericia()) {
			p.setPericia(p.getPericia() + 1);
			System.out.println("Poder: " + p.getNome() + " aumentou em pericia!");
		}
	...
	public void evolucaoDefesa(Integer defesa, Lutador a) {
		...
		if (cf <= p.getChanceAgilidade()) {
			...
			System.out.println("Poder: " + p.getNome() + " aumentou em agilidade!");
		}

		if (cp <= p.getChanceResistencia()) {
			p.setResistencia(p.getResistencia() + 1);
			System.out.println("Poder: " + p.getNome() + " aumentou em resistencia!");
		}
...
//		a.getIventorio().getArmaduras()[0].ativar(a);
	...
	public void calcularDano(String forca, Lutador a, Integer multi) {
		...
		try {
			...
			Integer dano = (forc / a.getResistencia()) < 1 ? 1 : (forc / a.getResistencia());

			Integer danoTotal = (int) (dano + multi /*- a.getIventorio().getArmaduras()[0].getReducao()*/
					+ ((Math.random() * 4) + 1));
	...
	public void criarClasses() {
		...
		ClasseLutadora artistaMarcial = new ClasseLutadora("Artista Marcial", 2, 2, 6, 6);
